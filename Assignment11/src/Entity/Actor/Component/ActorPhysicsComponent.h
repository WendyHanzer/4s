...
class ActorPhysicsComponent : public Component
{
	private:
		...
		{
			setTransformation(((const Actor&)getController()).getTransformation());
	...
	protected:
		...
		inline void setTransformation(const glm::mat4& glTransformation)
		{
			glm::quat glRotation = glm::quat_cast(glTransformation);
			btVector3 position(glTransformation[3][0], glTransformation[3][1], glTransformation[3][2]);
			btQuaternion rotation(glRotation.x, glRotation.y, glRotation.z, glRotation.w);
